...
app.post("/send-bulk", async (req, res) => {
  ...
  return res.json({ total: emails.length, succeeded, failed: emails.length - succeeded, results, sentAt: new Date().toISOString() });
});
...
function buildHtmlEmail({ toName, subject, body, providerName, previewUrl }) {
  ...
  const demoButton = previewUrl ? `
    <div style="margin:28px 0;background:linear-gradient(135deg,#f0fffe,#e6f9f8);border:2px solid #00c4be;border-radius:14px;padding:24px 28px;text-align:center">
      ...
      <a href="${previewUrl}"
         style="display:inline-block;background:linear-gradient(135deg,#00c4be,#0288d1);color:white;padding:14px 36px;border-radius:50px;text-decoration:none;font-weight:800;font-size:15px;letter-spacing:0.3px;box-shadow:0 4px 20px rgba(0,196,190,0.35)">
...
<table width="600" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08);max-width:600px;width:100%">
  ...
  <tr><td style="padding:8px 40px 32px">
    <table cellpadding="0" cellspacing="0" style="border-top:1px solid #e2e8f0;padding-top:20px;width:100%">
    <tr><td>
  ...
  <tr><td style="background:#f7fafc;padding:16px 40px;border-top:1px solid #e2e8f0;text-align:center">
    ...
    <p style="margin:4px 0 0;font-size:11px;color:#a0aec0">To unsubscribe reply with "unsubscribe" in the subject.</p>
  </td></tr>
